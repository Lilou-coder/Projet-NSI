...
from helpers import apology, login_required,verify_answer
...
def joingame():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("game_code"):
            return apology("Veuillez renter le code du jeu", 401)
...
def admingame(gamecode):
    ...
    if request.method == "POST":
        ...
        # If not player_id:
        if player_id == []:
            return apology ("Une erreur s est produite lors de la récupération des joueurs, veuillez réessayer", 403)
...
def game(gamecode):
    ...
    if game_progress[0]["actif_question"] != -1:
        ...
        # If it is not the last question, see if answer is correct
        if request.method == "POST":
    ...
    # Refresh page with new question and information
...
def results(gamecode):
    ...
    # Check if game code is valid
    if game == []:
        return apology ("Une erreur s est produite lors de la récuperation du jeu, veuillez reessayer", 404)
    ...
    for i in range(len(players)):
        ...
        # Add to list of player their score and status
...
def correction(gamecode):
    ...
    for i in range(len(question_ids)):
        ...
        # Add to list each question and its answer
...
def creategame():
    if request.method == "POST":
        ...
        if not request.form.get("subject"):
            return apology("Le sujet est manquant", 401)
